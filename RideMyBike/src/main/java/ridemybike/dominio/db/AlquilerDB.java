...
public class AlquilerDB {
     ...
     * @return un Alquiler con los datos del alquiler; null si no existe ningun
     * alquiler con el código especificado
     ...
     * se incluiran los viajes en proceso ni los alquileres archivados.
     *
     * @param nombreUsuario el nombre de usuario
     * @return lista con todos los alquileres del usuario
     */
    ...
    /**
     * Inicia el viaje especificado
     *
     * @param codigoAlquiler el codigo del alquiler
     * @ubicacionFinal la ubicacion de inicio del viaje
     * @return un entero positivo si la actualizacion ha tenido exito; 0 si ha
     * habido algun fallo
     */
    public static int iniciarViaje(int codigoAlquiler, String ubicacionInicial) {
        ConnectionPool pool = ConnectionPool.getInstance();
        Connection connection = pool.getConnection();
        PreparedStatement ps = null;
        String query;
        query = "UPDATE Alquiler SET horaInicial = ?, inicio = ? WHERE codigoAlquiler = ?";

        try {
            ps = connection.prepareStatement(query);
            Timestamp horaFinal = new Timestamp(System.currentTimeMillis());
            ps.setString(1, horaFinal.toString());
            ps.setString(2, ubicacionInicial);
            ps.setString(3, codigoAlquiler + "");
