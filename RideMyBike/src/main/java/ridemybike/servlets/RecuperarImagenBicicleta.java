 ...
 */
@WebServlet(name = "RecuperarImagenBicicleta", urlPatterns = {"/RecuperarImagenBicicleta"})
public class RecuperarImagenBicicleta extends HttpServlet {
    public void processRequest(HttpServletRequest request,HttpServletResponse response)throws ServletException, IOException {
        response.setContentType("image/jpg");
        OutputStream respuesta = response.getOutputStream();
        String idBici = request.getParameter("idBici");
        BicicletaDB.getImagen(Integer.parseInt(idBici), respuesta);
        respuesta.close();
